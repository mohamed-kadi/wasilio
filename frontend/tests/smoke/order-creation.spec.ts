...
test('merchant creates a COD order and continues to confirmation', async ({ page }) => {
  ...
  await expect(page.locator('select[name="city"]')).toContainText('Agadir');
  await expect(page.locator('select[name="city"]')).toContainText('Dakhla');
  await page.getByLabel('City', { exact: true }).selectOption('Agadir');
  await expect(page.getByLabel('Region', { exact: true })).toHaveValue('Souss-Massa');
  await expect(page.getByLabel('Postal code', { exact: true })).toHaveValue('80000');
  await page.getByLabel('City', { exact: true }).selectOption('Casablanca');
  await expect(page.getByLabel('Region', { exact: true })).toHaveValue('Casablanca-Settat');
  await expect(page.getByLabel('Postal code', { exact: true })).toHaveValue('20000');
